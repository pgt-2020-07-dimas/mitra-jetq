                    ...
                    @if($data <> null)
                        @foreach ($data as $d)
                            <tr class="invoice-row" data-id="{{ $d['invoice_id'] }}" id="{{ $d['invoice_id'] }}">
                                ...
                                <td>{{$d['payment_status'] == 4 ? 'Payment Success' : ''}}</td>
                            </tr>
                        @endforeach
                        @else
                        ...
                        <div class="row mb-2 row-button" style="display: none;">
                            <div class="col">
                                <form action="/transaction/payment" method="post">
                                    ...
                                    <button type="submit" class="btn btn-outline-success btn-block btn-complete" style="display: none;"><span class="fas fa-check-square"></span> Complete Order</button>
                                </form>
                            </div>
                        </div>
                        <!-- END ROW 8 -->
